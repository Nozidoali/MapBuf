...
def equivalence_checking(dfg1: pgv.AGraph, dfg2: pgv.AGraph, verbose: bool = False) -> bool:
    '''
    ...
    for node1 in dfg1.nodes():
        ...
        if not is_same(node1, node2, verbose):
            if verbose:
                print(f"Node {node1.get_name()} in DFG1 is not the same as node {node2.get_name()} in DFG2")
            return False
        ...
        for n in dfg1.predecessors(node1):
            ...
            if n not in dfg2.predecessors(node2):
                if verbose:
                    print(f"Node {node1.get_name()} in DFG1 has a predecessor {n.get_name()} that is not in DFG2")
                return False
        ...
        for n in dfg1.successors(node1):
            ...
            if n not in dfg2.successors(node2):
                if verbose:
                    print(f"Node {node1.get_name()} in DFG1 has a successor {n.get_name()} that is not in DFG2")
                return False
