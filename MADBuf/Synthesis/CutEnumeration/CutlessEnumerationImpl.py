...
def get_timing_labels(g: BLIFGraph, signal_to_channel: dict = {}, cut_size_limit: int = 6, max_expansion_level: int = 0) -> tuple:
    """Get timing labels
...
def cutless_enumeration_impl(
    ...
    **kwargs
    ...
    signal_to_channel = kwargs["signal_to_channel"] if "signal_to_channel" in kwargs else {}
    lut_size_limit = kwargs["lut_size_limit"] if "lut_size_limit" in kwargs else 6    
    verbose = kwargs["verbose"] if "verbose" in kwargs else False
    max_expansion_level = kwargs["max_expansion_level"] if "max_expansion_level" in kwargs else 0
    
    if signal_to_channel == None:
        signal_to_channel = {}

    labels, cuts = get_timing_labels(network, signal_to_channel, lut_size_limit, max_expansion_level)
