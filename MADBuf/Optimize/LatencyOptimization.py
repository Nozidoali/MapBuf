...
def add_channel_variables(model: gp.Model, g: BLIFGraph, signal_to_channel: dict) -> dict:
    ...
    for u, v, idx in all_component_pairs:
        ...
        model.addConstr(model.getVarByName(f"Channel_{channel_data}") == model.getVarByName(f"Channel_{channel_valid}"))
...
def add_latency_labels(model: gp.Model, g: BLIFGraph):
    ...
    for signal in g.signals:
        model.addVar(vtype=GRB.INTEGER, name=f"LatencyLabel_{signal}", lb=0)  # delay variables 
        
    model.update()
    
    # the latency is the maximum of all the latency labels
    for signal in g.signals:
        model.addConstr(model.getVarByName(f"LatencyLabel_{signal}") <= model.getVarByName(f"latency"))
        
    model.update()


def add_latency_propagation_constraints(model: gp.Model, g: BLIFGraph, signal_to_channel_var: dict):
    ...
    for signal in g.signals:
        ...
        if signal not in signal_to_channel_var:
            ...
            for leaf in g.node_fanins[signal]:
                model.addConstr(model.getVarByName(f"LatencyLabel_{leaf}") <= model.getVarByName(f"LatencyLabel_{signal}"))
...
def add_timing_constraints_for_latency_optimization(
    ...
    signal_to_channel_var: dict = add_channel_variables(model, network, signal_to_channel)
