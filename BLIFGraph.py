    ...
    def retrieve_anchors(self) -> tuple:
        ...
        for n in self.outputs:
            ...
            if "__in" in n:
                ...
                #
                component = c.v if c.t == Constant._channel_ready_ else c.u
        ...
        for n in self.inputs:
            ...
            if c is None:
                continue
            component = c.u if c.t == Constant._channel_ready_ else c.v
            if "__in" in n:
...
def read_graph_from_blif(filename: str) -> "BLIFGraph":
    g: BLIFGraph = BLIFGraph()
    ...
    for module in modules:
        ...
        for line in modules[module]:
            ...
            if line.startswith(".output"):
                on_output(_g, line)
        g.submodules[module] = _g

    index: int = 0
    while True:
        assert index < len(modules[g.top_module]) and "index out of range"
        line = modules[g.top_module][index]
        if line.startswith(".end"):
            ...
            while True:
                index += 1
                _nextline: str = modules[g.top_module][index].strip()
                if len(_nextline) == 0:
                    break
