...
for row in rows:
    ...
    logic_delays, wire_delays, lut_level_delays, adder_delays = run_delay_analysis(setup_timing_report_path=setup_path, n_paths_to_analyze=50, verbose=False)
...
names = ["Logic", "Wire", "LUT level", "DSP"]
xs = []
ngroup = len(vals)
...
for i in range(ngroup):
    plt.text(i+1+0.23, means[i], f"{means[i]:0.02f}", ha='left', va='center', color='black', fontsize=15)
...
ax1.legend(handles=legend_elements, loc='upper left', fontsize=15)
