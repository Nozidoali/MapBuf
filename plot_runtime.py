...
def plot(benchmarks: list):
    ...
    plt.plot([cutless_conv_time,cutless_conv_time], [0,cutless_conv_value], '-.k', color=colors[0], linewidth=1.5, alpha=0.75)  # solid green

    heu_5_conv_time = 208.00837922096252 / 60
    heu_5_conv_value = 0.37153043964 / 0.2436812754166485
    plt.plot([heu_5_conv_time,heu_5_conv_time], [0,heu_5_conv_value], '-.k', color=colors[1], linewidth=1.5, alpha=0.75)  # solid green

    dac_conv_time = 11.564226865768433 / 60
    dac_conv_value = 0.24704829516385102 / 0.2436812754166485
    plt.plot([dac_conv_time,dac_conv_time], [0,dac_conv_value], '-.k', color=colors[2], linewidth=1.5, alpha=0.75)  # solid green

    fpl_conv_time = 171.50699496269226 / 60
    fpl_conv_value = 0.24362750845694794 / 0.2436812754166485
    plt.plot([fpl_conv_time,fpl_conv_time], [0,fpl_conv_value], '-.k', color=colors[3], linewidth=1.5, alpha=0.75)  # solid green
