...
class L:
    ...
    def __le__(self, other: "L") -> bool:
        return not self.__gt__(other)

    def __gt__(self, other: "L") -> bool:
        return self.s > other.s or (self.s == other.s and self.d > other.d)
...
class MAPS:
    def __init__(self, g: BLIFGraph, clock_period: int) -> None:
        ...
        for n in self.g.topological_traversal():
            if self.g.is_ci(n):
                self.labels[n] = L(0)  # 0, but this is not necessary
            else:
                self.labels[n] = L()  # inf

        self.clock_period: int = clock_period
    ...
    def run(
        ...
        l_max, n_max = max([(self._node_arrival_time(n), n) for n in self.g.signals])
        print(f"[i] propagation delay = {str(l_max)}")
        if l_max > L(self.clock_period):
    ...
    def _update_label(self, n: str) -> L:
        ...
        l_opt: L = self._node_arrival_time(n)
        assert n in self.g.node_fanins

        leaves: set = set(list(self.g.node_fanins[n])[:])  # deep copy
        assert len(leaves) <= Constant.lut_size_limit and "failed to find initial cut"
        while len(leaves) <= Constant.lut_size_limit:
            arrival_times: list = [(self._node_arrival_time(f), f) for f in leaves]
            l_max, f = max(arrival_times)

            l_opt = min(l_max + 1, l_opt)
    ...
    def _reduce_arrival_time_to(self, n: str, timing_constraint: L = None) -> bool:
        if timing_constraint == None:
            timing_constraint = L(self.clock_period)  # default constraint
    ...
    def _fix_timing_violation(
        ...
        l_new = self._fix_timing_violation_helper(
            n,
            timing_constraint=L(self.clock_period),
            recursive_fix=recursive_fix,
            verbose=verbose,
            very_verbose=very_verbose,
        )

        if l_new <= L(self.clock_period):
        ...
        if very_verbose:
            if self._node_arrival_time(n) > L(self.clock_period):
    ...
    def _fix_timing_violation_helper(
        ...
        if (
            ...
            or len(self.g.node_fanins[n]) > Constant.lut_size_limit
        ):
        ...
        while len(leaves) <= Constant.lut_size_limit:
            ...
            if verbose:
                ...
                print(
                    f"considering node: {f:>20}, l_max = {text_red(l_max):>2}, cut size = {len(leaves)}, cut = \n\t{cut_str}"
                )
    ...
    def _node_arrival_time(self, n: str) -> L:
        if n in self.dirty_labels:
            return self.dirty_labels[n]
        return self.labels[n]
        # return L(0) if self._node_is_buffered(n) else self.labels[n]
...
if __name__ == "__main__":
    ...
    # anchor test
    m = MAPS(filename="./tests/teeny.blif", clock_period=4)
    m.run()
    pass
